...
doc is feature-complete across milestones M0 through M9. It is pre-1.0, so the library API is still settling toward 1.0; see the [stability](/reference/stability/) page for what that means.

What a `.doc` file and binary give you:
...
- **A versioned, forward-checked file format.** The header records a format major and minor version. A file whose major version is newer than the build is rejected with a clear error instead of misread, so an upgrade is safe.
...
- **M9** hardened fuzz corpora, `$lookup` with a pipeline, the columnar projection store, crash and recovery at scale, the performance polish, and this first release.
